...
def train():
  ...
  #step l.l
  target = tf.one_hot(tf.argmin(c_prediction,1), num_label, on_value=1.,off_value=0.)
  loss=tf.nn.softmax_cross_entropy_with_logits(labels=target,logits=c_net);
  dy_dx,=tf.gradients(loss,img);
  x_adv = tf.stop_gradient(img -1*eps*tf.sign(dy_dx));
...
if __name__ == '__main__':
  ...
  parser.add_argument('--max_epsilon', type=int, default=45,
                      help='max epsilon')
  parser.add_argument('--max_iter', type=int, default=5,
                      help='max iter on iterative GSM (If this one is 1, then step l.l)')
  parser.add_argument('--batch_num', type=int, default=1000,
                      help='the batch size')
  parser.add_argument('--max_epoch', type=int, default=100,
                      help='the max iteration')
  parser.add_argument('--tb_folder', type=str, default="/tmp/adv_training_cons/adv_training_stepll",
  ...
  parser.add_argument('--save_file_name', type=str, default="adv_trained_stepll.ckpt",
